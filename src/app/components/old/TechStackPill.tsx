...
export default function TechStackPill(props: { data: TechStack; type: 1 | 2 }) {
    const classType = props.type === 1 ? "border rounded-full text-white" : "bg-slate-300 text-slate-950 rounded-lg";
    const icon: JSX.Element =
        props.data.iconVar === "internal" ? (
            <Image
                ...
                width={32}
                height={32}
                className={`mx-auto min-w-[32px] h-full ${props.type === 2 ? "brightness-0" : ""}`}
            />
        ) : (
            <Image
                ...
                height={24}
                unoptimized={true}
                className={`${props.type === 1 ? "brightness-0 invert" : "brightness-0"}`}
            />
        );
    return (
        <div
