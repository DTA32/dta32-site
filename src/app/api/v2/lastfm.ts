...
import { getTrackArtwork } from "./itunes";
...
const fetchRecentTracks: ({
  ...
  limit?: number;
}) => Promise<ResponseTemplate<any>> = async ({ limit = 10 }) => {
...
const searchSpotifyTrack: ({
  ...
  title: string;
  artist: string;
}) => Promise<any> = async ({ songId, title, artist }) => {
  const track = await searchTrack({ track: title, artist: artist });
  if (
    !track.data ||
    !track.data.tracks ||
    track.data.tracks.items.length === 0 ||
    !track.data.tracks.items[0] ||
    !track.data.tracks.items[0].album ||
    !track.data.tracks.items[0].album.images ||
  ...
  const songUrl =
    track.data.tracks.items[0].external_urls &&
...
const searchTrackArtworkItunes: (
  songId: string,
  title: string,
  artist: string,
) => Promise<any> = async (songId, title, artist) => {
  const imageUrl = await getTrackArtwork(title, artist);
  if (imageUrl.status === "error") {
    return { songId: songId, imageUrl: null };
  }
  return { songId: songId, imageUrl: imageUrl.data };
};
...
const getTopTracks: () => Promise<ResponseTemplate<TrackItem[]>> = async () => {
  ...
  // as lastfm cant return image, search from itunes instead
  const [...trackImage] = await Promise.all(
    data.toptracks.track.map(async (track: any) => {
      const songId = track.url ? track.url : track.name;
      const artistName =
        track.artist && track.artist.name ? track.artist.name : "";
      const trackName = track.name ? track.name : "";
      return searchTrackArtworkItunes(songId, trackName, artistName);
    }),
  );

  const tracks: TrackItem[] = data.toptracks.track.map(
    (track: any, index: number) => {
      const imageData = trackImage.find(
        (img: any) => img.songId === (track.url ? track.url : track.name),
      );
      const imageUrl = imageData ? imageData.imageUrl : "";
      const artistName =
        track.artist && track.artist.name ? track.artist.name : "";
      const trackName = track.name ? track.name : "";
      const trackUrl = track.url ? track.url : "";
      return {
        rank: index + 1,
        imageUrl: imageUrl,
        title: trackName,
...
> = async () => {
  ...
  const albumImageUrl =
    nowPlaying.image && nowPlaying.image.length > 0
      ? nowPlaying.image
          .map((img: any) => img)
          .filter((img: any) => img.size === "extralarge")[0]["#text"]
      : "";
  ...
  const artistUrl = nowPlaying.artist
